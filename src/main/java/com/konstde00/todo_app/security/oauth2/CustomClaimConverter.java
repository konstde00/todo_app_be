package com.konstde00.todo_app.security.oauth2;
...
public class CustomClaimConverter implements Converter<Map<String, Object>, Map<String, Object>> {
  ...
  private final Cache<String, ObjectNode> users =
      Caffeine.newBuilder()
          .maximumSize(10_000)
  ...
  public Map<String, Object> convert(Map<String, Object> claims) {
    ...
    if (attributes instanceof ServletRequestAttributes) {
      ...
      String token =
          bearerTokenResolver.resolve(((ServletRequestAttributes) attributes).getRequest());
      ...
      ObjectNode user =
          users.get(
              ...
              s -> {
                ResponseEntity<ObjectNode> userInfo =
      ...
      if (user != null) {
        ...
        if (user.has("name")) {
          ...
          if (name.length > 0) {
            ...
            convertedClaims.put(
                "family_name", String.join(" ", Arrays.copyOfRange(name, 1, name.length)));
          }
        }
        if (user.has("groups")) {
          List<String> groups =
              StreamSupport.stream(user.get("groups").spliterator(), false)
                  .map(JsonNode::asText)
                  .toList();
        ...
        if (user.has(SecurityUtils.CLAIMS_NAMESPACE + "roles")) {
          List<String> roles =
              StreamSupport.stream(
                      user.get(SecurityUtils.CLAIMS_NAMESPACE + "roles").spliterator(), false)
                  .map(JsonNode::asText)
                  .toList();
